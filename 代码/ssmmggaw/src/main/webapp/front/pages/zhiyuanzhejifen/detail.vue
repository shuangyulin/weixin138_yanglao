	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback" :bottom="100">
		<view class="container">
				...
				<swiper-item v-for="(swiper,index) in swiperList" :key="index">
					...
					<view class="lable" :style='{"width":"180rpx","fontSize":"28rpx","color":"#333","textAlign":"left"}'>志愿者积分：</view>
					<view class="text" :style='{"color":"#333","textAlign":"left","fontSize":"28rpx"}'>{{detail.zhiyuanzhejifen}}</view>
				</view>
                				                								                				<view class="detail-list-item" :style='{"boxShadow":"","margin":"","borderColor":"#fff","backgroundColor":"rgba(255, 255, 255, 0.56)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"68rpx"}'>
...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','zhiyuanzhejifen');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','zhiyuanzhejifen');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('zhiyuanzhejifen', this.id);
				this.detail = res.data;
																																				// 轮播图片
				this.swiperList = this.detail.jifentu ? this.detail.jifentu.split(",") : [];
																			},
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discusszhiyuanzhejifen/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('zhiyuanzhejifen', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('zhiyuanzhejifen', this.detail);
				this.$utils.msg('审核成功');
