...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussyanglaozhengce');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','discussyanglaozhengce');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('discussyanglaozhengce', this.id);
			...
			async upCallback(mescroll) {
				                let res = await this.$api.list('discussdiscussyanglaozhengce', {
					page: mescroll.num,
					limit: mescroll.size,
					refid: this.id
				});
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussdiscussyanglaozhengce/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('discussyanglaozhengce', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('discussyanglaozhengce', this.detail);
				this.$utils.msg('审核成功');
