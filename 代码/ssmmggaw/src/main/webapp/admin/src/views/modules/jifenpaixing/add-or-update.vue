<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.paixingtu" label="排行图" prop="paixingtu">
          <file-upload
          tip="点击上传排行图"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.paixingtu?ruleForm.paixingtu:''"
          @change="paixingtuUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.paixingtu" label="排行图" prop="paixingtu">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.paixingtu.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="排行表" prop="paixingbiao">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.paixingbiao" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.paixingbiao" label="排行表" prop="paixingbiao">
                    <span v-html="ruleForm.paixingbiao"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.yuefen = true;
          ...
          if(o=='paixingbiao'){
            this.ruleForm.paixingbiao = obj[o];
	    this.ro.paixingbiao = true;
            continue;
          }
          if(o=='paixingtu'){
            this.ruleForm.paixingtu = obj[o];
	    this.ro.paixingtu = true;
            continue;
          }
        }
      }
	...
	this.ruleForm.paixingbiao = this.ruleForm.paixingbiao.replace(reg,'../../../ssmmggaw/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `jifenpaixing/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jifenpaixingCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    paixingtuUploadChange(fileUrls) {
	this.ruleForm.paixingtu = fileUrls;
			this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
