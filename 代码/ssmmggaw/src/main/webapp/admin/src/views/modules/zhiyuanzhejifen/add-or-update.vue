<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="志愿者积分" prop="zhiyuanzhejifen">
          <el-input v-model="ruleForm.zhiyuanzhejifen" 
              placeholder="志愿者积分" clearable  :readonly="ro.zhiyuanzhejifen"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="志愿者积分" prop="zhiyuanzhejifen">
              <el-input v-model="ruleForm.zhiyuanzhejifen" 
                placeholder="志愿者积分" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="志愿号" prop="zhiyuanhao">
          <el-select  @change="zhiyuanhaoChange" v-model="ruleForm.zhiyuanhao" placeholder="请选择志愿号">
            <el-option
                v-for="(item,index) in zhiyuanhaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.zhiyuanhao" label="志愿号" prop="zhiyuanhao">
              <el-input v-model="ruleForm.zhiyuanhao" 
                placeholder="志愿号" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.jifentu" label="积分图" prop="jifentu">
          <file-upload
          tip="点击上传积分图"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.jifentu?ruleForm.jifentu:''"
          @change="jifentuUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.jifentu" label="积分图" prop="jifentu">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.jifentu.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    zhiyuanhaoChange () {
      this.$http({
        url: `follow/zhiyuanzhe/zhiyuanhao?columnValue=`+ this.ruleForm.zhiyuanhao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `zhiyuanzhejifen/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.zhiyuanzhejifenCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    jifentuUploadChange(fileUrls) {
	this.ruleForm.jifentu = fileUrls;
			this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
